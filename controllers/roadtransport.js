...
// Handle building the view for creating a roadtransport.
// No body, no in path parameter, no query.
// Does not need to be async
exports.roadtransport_create_Page = function(req, res) {
    console.log("create view")
    try{
    res.render('roadtransportcreate', { title: 'Roadtransport Create'});
    }
    catch(err){
    res.status(500)
    res.send(`{'error': '${err}'}`);
...
// Handle building the view for updating a roadtransport.
// query provides the id
exports.roadtransport_update_Page = async function(req, res) {
    console.log("update view for item "+req.query.id)
    try{
    let result = await roadtransport.findById(req.query.id)
    res.render('roadtransportupdate', { title: 'roadtransport Update', toShow: result });
    }
    catch(err){
    res.status(500)
    res.send(`{'error': '${err}'}`);
...
exports.roadtransport_delete_Page = async function(req, res) {
    console.log("Delete view for id " + req.query.id)
    try{
    result = await roadtransport.findById(req.query.id)
    res.render('roadtransportdelete', { title: 'roadtransport Delete', toShow:
   result });
    }
    catch(err){
    res.status(500)
